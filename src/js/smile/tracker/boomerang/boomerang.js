...
boomr = {
    ...
    sendBeacon: function() {
        ...
        url = impl.beacon_url + ((impl.beacon_url.indexOf('?') > -1)?'&':'?') +
            'page.u=' + encodeURIComponent(d.URL.replace(/#.*/, ''));
            // use d.URL instead of location.href because of a safari bug

        for(k in impl.vars) {
            if(impl.vars.hasOwnProperty(k)) {
                var paramName = k;
                if (!k.startsWith('page') && !k .startsWith('session')) {
                    paramName = 'page.rum.' + k;
                }
                nparams++;
                url += "&" + paramName
                    + "="
...
BOOMR.plugins.RT = {
    ...
    // load when the page is usable by the user
        ...
        if(ntimers) {
            BOOMR.addVar("page.r", r);

            if(r2 !== r) {
                BOOMR.addVar("page.r2", r2);
            }
